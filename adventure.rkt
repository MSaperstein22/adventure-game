...
(define-struct (iphone thing)
  (turn-on? locked? passcode)
  #:methods
  (define (turn-on c)
    (if (iphone-locked? c)
        (printf "The iphone is locked")
        (set-iphone-turn-on?! c true)))
  ; enter-passcode: iphone number -> void
  ; Unlock the iphone if the passcode matches the iphone's passcode.
  (define (enter-passcode p code)
    (if (equal? (iphone-passcode p) code)
        (begin (set-iphone-locked?! p false)
               (printf "Congratulations! You unlocked the iPhone. Now admit that you love racket."))
        (printf "Wrong Passcode"))
  ))


(define (new-iphone adjectives passcode location)
  (local [(define the-iphone (make-iphone (string->words adjectives)
                                        '() location
                                        false
                                        true
                                        passcode))]
    (begin (initialize-thing! the-iphone)
           the-iphone)))


(define-struct (photograph thing)
  (broken?)
  #:methods
  (define (break t)
        (begin (destroy! t) (set-photograph-broken?! t true)
           (display-line "Did you just break a 1985 Super Bowl Champions photograph? You don't deserve to win. You lose.") (start-game) (look)))
)

(define (new-photograph adjectives location)
  (local [(define the-photograph (make-photograph (string->words adjectives)
                                                  '() location
                                                  false))]
    (begin (initialize-thing! the-photograph)
           the-photograph)))
...
(define-struct (potion thing)
  ...
  (define (drink p)
    (if (is-a? p "potion")
        (begin (set-potion-finished?! p true)
               (destroy! p)
               (display-line "You died") (start-game) (look))
        (display-line "Drink the potion. You won't."))))

(define (new-potion adjectives location)
  (local [(define the-potion (make-potion (string->words adjectives)
                                                  '() location
                                                  false))]
    (begin (initialize-thing! the-potion)
           the-potion)))


(define-struct (Ian person)
  (secret-line)
  #:methods
  ; tell-Ian: Ian string -> void
  ; Win the game if the secret line matches Ian's secret line.
  (define (tell s my-line)
    (if (and (equal? (Ian-secret-line s) my-line) (equal? (iphone-locked? (the iphone)) false))
        (begin
               (printf "I knew you would love it! Congratulations! You won the game. Type (start-game) and (look) to play again"))
        (if (equal? (iphone-locked? (the iphone)) true)
            (printf "Unlock the iphone first. Then we'll talk.")
            (printf "Tell me the truth!")))
  ))

(define (new-Ian adjectives secret-line location)
  (local [(define the-Ian (make-Ian (string->words adjectives)
                                        '() location
                                        secret-line))]
    (begin (initialize-thing! the-Ian)
           the-Ian)))


(define-struct (lightswitch thing)
  (light-room?)
  #:methods
  (define (flip-on c)
    (begin (set-lightswitch-light-room?! c true) (printf "The light is on")))
  (define (flip-off c)
    (begin (set-lightswitch-light-room?! c false) (printf "The light is off")))
  )


(define (new-lightswitch adjectives location)
  (local [(define the-lightswitch (make-lightswitch (string->words adjectives)
                                        '() location
                                        false))]
    (begin (initialize-thing! the-lightswitch)
           the-lightswitch)))
...
(define-user-command (turn-on thing)
  "Turns on the iphone")

(define-user-command (enter-passcode thing code)
  "Enters the iphone passcode")

(define-user-command (break thing)
  "Breaks something.")
...
(define (smell thing)
  (if (is-a? thing "potion")
    (begin (display-line "Something smells funny..."))
    (display-line "I can't smell anything.")))

(define-user-command (smell thing)
  "Smells the thing.")

(define-user-command (tell person my-line)
  "Tell Ian the secret line.")

(define (say-hi person)
  (if (is-a? person "Ian")
    (begin (display-line "Hello there. Make sure to watch the lectures before coming to class!"))
    (display-line "I can't say hi to that.")))

(define-user-command (say-hi person)
  "Says hi to a person.")


(define-user-command (flip-on thing)
  "Flips on the lightswitch")

(define-user-command (flip-off thing)
...
(define (start-game)
  ...
  (local [(define starting-room (new-room ""))]
    (begin (set! me (new-person "" starting-room))
           ...
           (new-iphone "brand new" 1985 starting-room)
           (new-photograph "1985 Super Bowl Champions" starting-room)
           (new-potion "magic" starting-room)
           (new-Ian "person named" "I love racket" starting-room)
           (new-lightswitch "" starting-room)
           (check-containers!)
           (void))))
...
(define-walkthrough win
  (enter-passcode (the iphone) 1985)
  (tell (the Ian) "I love racket"))
